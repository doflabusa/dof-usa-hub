...
function App() {
  ...
  const activeTasks = useMemo(() => {
    return sortTasksByDueDate(visibleTasks.filter(t => (t.status || 'To Do') !== 'Completed'));
  }, [visibleTasks]);
  ...
  function assignedDate(row) {
    return row.created_at ? new Date(row.created_at).toLocaleDateString() : '-';
  }

  function taskDueClass(row) {
    if ((row.status || 'To Do') === 'Completed' || !row.due_date) return '';
  ...
  function sortTasksByDueDate(rows) {
            ...
            <div className="two">
              ...
              <Card title="Active / Assigned Tasks">
                {activeTasks.length ? activeTasks.map(t=>
                  <Task key={t.id} t={t} currentUserId={session.user.id} isAdmin={isAdmin} updateTaskStatus={updateTaskStatus} deleteTask={deleteTask}/>
...
function Task({t, currentUserId, isAdmin, updateTaskStatus, deleteTask, compact=false}) {
  ...
  return (
    <div className={"task" + dueClass}>
      ...
      <div>
        <b>{t.title}</b>
        <span>{t.assignee_name || 'Unassigned'} · {t.priority || 'Medium'}</span>
        <div className="taskDates">
          <small>Assign Date: {assignedDate(t)}</small>
          <small>Due Date: {t.due_date || 'No due date'}</small>
        </div>
