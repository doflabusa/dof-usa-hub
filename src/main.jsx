...
function App() {
  ...
  const [task,setTask] = useState({ title:'', assignee_id:'', assignee_name:'', priority:'Medium', due_date:'', description:'' });
  ...
  async function loadAll() {
    ...
    const [a,t,e,ps] = await Promise.all([
    ...
    if(!task.assignee_id && profileRows.length) {
      setTask(prev => ({...prev, assignee_id: profileRows[0].id, assignee_name: profileRows[0].full_name}));
    }
  }
  ...
  async function clockOut() {
    ...
    const res = await supabase.from('attendance_records')
      .update({ clock_out_at:clockOutAt, status:'completed' })
      .eq('id', open.id);
    if(res.error) return setError('Clock Out error: ' + res.error.message);
    setNotice(`Clock Out completed. Today's work time: ${hoursBetween(open.clock_in_at, clockOutAt)}`);
    await loadAll();
  }
  ...
  async function createTask(e) {
    ...
    setTask({ title:'', assignee_id:assignee.id, assignee_name:assignee.full_name, priority:'Medium', due_date:'', description:'' });
  ...
  function exportCSV() {
    ...
    csv([
      ['Employee','Clock In','Clock Out','Total Hours','Timezone','HQ Timezone','Status'],
  ...
  if(!session) {
    return (
      <div className="login">
        <div className="hero">
          <h1>DOF USA HUB</h1>
          <p>Internal operations portal for attendance, tasks, and calendar management.</p>
        </div>
  ...
  return (
    <div className="app">
      ...
      <main>
        ...
        {page==='Dashboard' &&
          <Dashboard open={open} tasks={tasks} events={events} clockIn={clockIn} clockOut={clockOut} setPage={setPage} month={month} setMonth={setMonth} />
        }
        ...
        {page==='Attendance' &&
          <Card title="Attendance Records" action={
            ...
            <Table headers={['Employee','Clock In','Clock Out','Total Hours','Timezone','Status']}
            ...
            <Card title="Create Event">
              <form onSubmit={createEvent} className="eventForm">
                ...
                <select value={event.type} onChange={e=>setEvent({...event,type:e.target.value})}>
                  <option>Company</option><option>Personal</option><option>Field Work</option><option>Installation</option><option>Business Trip</option>
                </select>
...
function Dashboard({open,tasks,events,clockIn,clockOut,setPage,month,setMonth}) {
  return (
    <>
      <section className="kpis">
        ...
        <KPI title="Open Tasks" value={tasks.filter(t=>t.status!=='Completed').length} icon="📌"/>
      </section>
      <div className="two">
        <Card title="Attendance & Location" action={<button onClick={open?clockOut:clockIn}>{open?'Clock Out':'Clock In'}</button>}>
          <div className="clock">
            ...
            <span>{open?'Checked in':'Not clocked in'}<br/>Timezone: {open?.local_timezone||localTz()}</span>
          </div>
        </Card>
