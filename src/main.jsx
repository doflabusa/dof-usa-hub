...
function randomQuote() {
  return monthlyQuotes[Math.floor(Math.random() * monthlyQuotes.length)];
}
  ...
  return (
    <div className="app">
      ...
      <main>
        ...
        {page==='Dashboard' && <Dashboard quote={dashboardQuote} open={open} tasks={visibleTasks} events={events} leaves={visibleLeaves} startShift={startShift} endShift={endShift} setPage={setPage} month={month} setMonth={setMonth} />}
        ...
        {page==='Leave' && <>
          ...
          {isAdmin && <Card title="Set Initial Leave Balance">
            <p className="hint">Enter the employee's available leave balance in hours. Example: 10 days = 80 hours based on an 8-hour workday.</p>
            <form onSubmit={saveLeaveBalance} className="leaveBalanceForm">
              ...
              <label>Paid Time Off Balance (hours)
                <input type="number" step="0.5" placeholder="Example: 80" value={balanceForm.paid_time_off_hours} onChange={e=>setBalanceForm({...balanceForm,paid_time_off_hours:e.target.value})}/>
              </label>
              <label>Paid Sick Leave Balance (hours)
                <input type="number" step="0.5" placeholder="Example: 40" value={balanceForm.paid_sick_leave_hours} onChange={e=>setBalanceForm({...balanceForm,paid_sick_leave_hours:e.target.value})}/>
              </label>
        ...
        {page==='Admin' && isAdmin && <Card title="Admin Tools"><div className="actions"><button onClick={exportCSV}>Export Attendance CSV</button><button className="light" onClick={loadAll}>Sync Latest Data</button></div></Card>}
      </main>
    </div>
  );
}
        ...
        <div className="weekBlock" key={wi}>
          ...
          <div className="weekEvents">
            {weekSegments(days).map(({event,start,span}) => (
              <div
                key={event.id + '-' + wi}
                className={`eventBar ${eventClass(event.type)}`}
                style={{ gridColumn: `${start + 1} / span ${span}` }}
                title={`Created by ${event.created_by_name || 'Unknown'}`}
              >
