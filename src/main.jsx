...
import CostPricingModule from './pricing.jsx';
import './styles.css';
...
const pricingMenu = [['💵','Cost & Pricing']];
  ...
  return (
    <div className="app">
      <aside>
        ...
        <nav>{[...baseMenu, ...(isAdmin ? adminMenu : []), ...((isAdmin || isSales) ? pricingMenu : [])].map(([ico,name])=>
      ...
      <main>
        ...
        {page==='Cost & Pricing' && (isAdmin || isSales) &&
          <CostPricingModule supabase={supabase} session={session} profile={profile} />
        }
      </main>
    </div>
  );
