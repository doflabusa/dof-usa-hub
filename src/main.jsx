          ...
          <Card title="U.S. Installation Map">
            <div className="mapPreview">
              <div className="mapBox realMapBox">
                ...
                <div className="mapSub">State-level installation visualization based on Installed Products data.</div>
                <ComposableMap projection="geoAlbersUsa" className="usaMap">
                  ...
                  {mapMarkers().map(m => (
                    <Marker key={m.state} coordinates={m.coords}>
                      <circle r={Math.min(16, 5 + m.count * 2)} className="mapDot" />
                      <text y={4} textAnchor="middle" className="mapLabel">{m.state}</text>
                    </Marker>
                  ))}
                </ComposableMap>
              </div>
              <div className="mapStats">
                ...
                <Card title="Top States">
                  {stateSummary().length ? stateSummary().slice(0,8).map(([state,count])=><div className="stateRow" key={state}><span>{state}</span><b>{count}</b></div>) : <Empty text="No installed product data yet."/>}
                </Card>
              </div>
            </div>
          </Card>
          ...
          <>
            <Card title={editingInstalledId ? "Edit Installed Product" : "Add Installed Product"}>
              <form onSubmit={createInstalledProduct} className="opsForm labeledOpsForm">
                ...
                <label>Product Model
                  <input placeholder="CRAFT 2" value={installedForm.product_model} onChange={e=>setInstalledForm({...installedForm,product_model:e.target.value})}/>
                </label>
                <label>Serial Number
            ...
            <Card title="Installed Products List">
              <Table headers={['Model','Serial','Customer','City','State','Installed','Warranty End','Status','Action']} rows={installedProducts.map(p=>[p.product_model,p.serial_number,p.customer_name,p.city,p.state,p.install_date,p.warranty_end,p.status,
