...
function App() {
  ...
  async function createInstalledProduct(e) {
    ...
    const res = await supabase.from('installed_products').insert(row);
    if (res.error) return setError('Installed Product create error: ' + res.error.message);
    setInstalledForm({
      product_category:'Milling Machine', product_model:'CRAFT 2', serial_number:'', customer_name:'',
      customer_type:'Lab', dealer_name:'', address:'', city:'', state:'CA', zip_code:'',
      ship_date:'', install_date:'', warranty_start:'', warranty_end:'', status:'Installed', notes:''
    });
    setNotice('Installed product added.');
    await loadAll();
  }
  ...
  return (
    <div className="app">
      ...
      <main>
        ...
        {page==='Map Preview' &&
          <Card title="U.S. Installation Map Preview">
            <div className="mapPreview">
              <div className="mapBox">
                <div className="mapTitle">DOF USA Installed Base</div>
                <div className="mapSub">Map visualization will use State / City / Product / Serial data.</div>
                <div className="fakeMapGrid">
                  {stateSummary().slice(0,12).map(([state,count])=><span key={state} className="statePill">{state}<b>{count}</b></span>)}
                  {!stateSummary().length && <div className="empty">No installed product data yet.</div>}
                </div>
              </div>
              <div className="mapStats">
                <KPI title="Installed Units" value={installedProducts.length} icon="📍"/>
                <KPI title="Active States" value={stateSummary().length} icon="🗺️"/>
        ...
        {page==='Installed Products' &&
          <>
            <Card title="Add Installed Product">
              <form onSubmit={createInstalledProduct} className="opsForm">
                <select value={installedForm.product_category} onChange={e=>setInstalledForm({...installedForm,product_category:e.target.value})}><option>Milling Machine</option><option>Intraoral Scanner</option><option>Desktop Scanner</option><option>Accessory</option></select>
                <input placeholder="Product Model" value={installedForm.product_model} onChange={e=>setInstalledForm({...installedForm,product_model:e.target.value})}/>
                ...
                <input placeholder="State" value={installedForm.state} onChange={e=>setInstalledForm({...installedForm,state:e.target.value.toUpperCase()})}/>
                ...
                <input type="date" title="Ship Date" value={installedForm.ship_date} onChange={e=>setInstalledForm({...installedForm,ship_date:e.target.value})}/>
                <input type="date" title="Install Date" value={installedForm.install_date} onChange={e=>setInstalledForm({...installedForm,install_date:e.target.value})}/>
                <input type="date" title="Warranty Start" value={installedForm.warranty_start} onChange={e=>setInstalledForm({...installedForm,warranty_start:e.target.value})}/>
                <input type="date" title="Warranty End" value={installedForm.warranty_end} onChange={e=>setInstalledForm({...installedForm,warranty_end:e.target.value})}/>
            ...
            <Card title="Installed Products List">
              <Table headers={['Model','Serial','Customer','City','State','Installed','Warranty End','Status']} rows={installedProducts.map(p=>[p.product_model,p.serial_number,p.customer_name,p.city,p.state,p.install_date,p.warranty_end,p.status])}/>
            </Card>
          </>
        }
