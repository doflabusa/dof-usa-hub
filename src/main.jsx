          ...
          <>
            ...
            <Card title="Installed Products List">
              <Table headers={['Model','Serial','Customer','PO','Invoice','TeamViewer','City','State','Installed','Warranty End','Support','Status','Action']} rows={installedRowsForList().map(p=>[
                ...
                <div className="actions" key={p.id}><button className="light" onClick={()=>editInstalledProduct(p)}>Edit</button>{isAdmin && <button className="miniDelete" onClick={()=>deleteInstalledProduct(p.id)}>×</button>}</div>
              ])}/>
            </Card>

            <Card title="Support History">
              <Table headers={['Date','Serial','Customer','Product','Type','Summary','Handled By']} rows={supportHistory.map(s=>[s.support_date,s.serial_number,s.customer_name,s.product_model,s.support_type,s.support_summary,s.handled_by_name])}/>
            </Card>
          </>
        }
