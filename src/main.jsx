...
function App() {
  ...
  const [event,setEvent] = useState({ title:'', event_date:'', end_date:'', event_time:'', all_day:true, type:'Company', notes:'' });
  const [leave,setLeave] = useState({ leave_type:'PTO', start_date:'', end_date:'', reason:'' });
  ...
  async function loadAll() {
    ...
    const [a,t,e,ps,l] = await Promise.all([
  ...
  async function createEvent(e) {
    ...
    setEvent({ title:'', event_date:'', end_date:'', event_time:'', all_day:true, type:'Company', notes:'' });
  ...
  async function createLeave(e) {
    ...
    setLeave({ leave_type:'PTO', start_date:'', end_date:'', reason:'' });
    setNotice('Leave request submitted.');
    await loadAll();
  }

  async function updateLeaveStatus(id, status) {
    const res = await supabase.from('leave_requests').update({ status }).eq('id', id);
  ...
  return (
    <div className="app">
      ...
      <main>
        ...
        {page==='Dashboard' &&
          <Dashboard open={open} tasks={tasks} events={events} leaves={leaves} startShift={startShift} endShift={endShift} setPage={setPage} month={month} setMonth={setMonth} />
        }
            ...
            <Card title="Create Event">
              <form onSubmit={createEvent} className="eventForm">
                ...
                <select value={event.type} onChange={e=>setEvent({...event,type:e.target.value})}>
                  <option>Business Trip</option><option>Installation</option><option>Company</option><option>Personal</option><option>Training</option>
                </select>
                <button>Create Event</button>
              </form>
              <div className="legend"><span className="trip">Business Trip</span><span className="install">Installation</span><span className="company">Company</span><span className="personal">Personal</span></div>
            </Card>
            ...
            <Card title="Request Leave">
              <form onSubmit={createLeave} className="form">
                <select value={leave.leave_type} onChange={e=>setLeave({...leave,leave_type:e.target.value})}>
                  <option>PTO</option><option>Paid Sick Leave</option><option>Unpaid Leave</option>
                </select>
            ...
            <Card title="Leave Requests">
              <Table headers={isAdmin?['Employee','Type','Start','End','Status','Action']:['Type','Start','End','Status']}
                rows={visibleLeaves.map(r=> isAdmin ? [r.employee_name,r.leave_type,r.start_date,r.end_date,r.status,
                  r.status==='Pending' ? <LeaveActions key={r.id} id={r.id} updateLeaveStatus={updateLeaveStatus}/> : '-'
...
function LeaveActions({id, updateLeaveStatus}) {
  return <div className="actions"><button onClick={()=>updateLeaveStatus(id,'Approved')}>Approve</button><button className="dark" onClick={()=>updateLeaveStatus(id,'Rejected')}>Reject</button></div>
}

function Dashboard({open,tasks,events,leaves,startShift,endShift,setPage,month,setMonth}) {
  return (
    <>
...
function eventClass(type) {
  ...
  if(type === 'Company' || type === 'Training') return 'company';
  return 'personal';
}
