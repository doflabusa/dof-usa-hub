          ...
          <Card title="U.S. Installation Map">
            ...
            <div className="mapPreview">
                ...
                <ComposableMap projection="geoAlbersUsa" className="usaMap">
                  <Geographies geography={GEO_URL}>
                    {({ geographies }) => geographies.map(geo => (
                      <Geography key={geo.rsmKey} geography={geo} className={"geoState" + (selectedMapState === STATE_NAME_TO_CODE[geo.properties.name] ? " selectedGeo" : "")} onClick={()=>setSelectedMapState(STATE_NAME_TO_CODE[geo.properties.name] || "")} />
                    ))}
                  </Geographies>
              ...
              <div className="mapStats">
                ...
                <Card title={selectedMapState ? `${selectedMapState} Installed Summary` : "State Summary"}>
                  {selectedMapState ? (
                    selectedMapStateSummary().length ? selectedMapStateSummary().map(([model,count])=><div className="stateRow" key={model}><span><i className="legendDot" style={{background:PRODUCT_COLORS[model] || '#64748b'}}></i>{model}</span><b>{count}</b></div>) : <Empty text="No installed products in this state."/>
                  ) : <Empty text="Click a state on the map to view installed product summary."/>}
                </Card>
              </div>
            </div>
          ...
          <>
            <Card title={editingInstalledId ? "Edit Installed Product" : "Add Installed Product"}>
              <form onSubmit={createInstalledProduct} className="opsForm labeledOpsForm">
                ...
                <label>PO Number
                  <input placeholder="PO Number" value={installedForm.po_number} onChange={e=>setInstalledForm({...installedForm,po_number:e.target.value})}/>
                </label>
                <label>Invoice Number
                  <input placeholder="Invoice Number" value={installedForm.invoice_number} onChange={e=>setInstalledForm({...installedForm,invoice_number:e.target.value})}/>
                </label>
                <label>TeamViewer ID
                  <input placeholder="TeamViewer ID" value={installedForm.teamviewer_id} onChange={e=>setInstalledForm({...installedForm,teamviewer_id:e.target.value})}/>
                </label>
                <label>TeamViewer Password
                  <input placeholder="Password / Access Code" value={installedForm.teamviewer_password} onChange={e=>setInstalledForm({...installedForm,teamviewer_password:e.target.value})}/>
                </label>
                <label>Customer Type
                ...
                <label>TeamViewer Notes
                  <input placeholder="TeamViewer notes" value={installedForm.teamviewer_notes} onChange={e=>setInstalledForm({...installedForm,teamviewer_notes:e.target.value})}/>
                </label>
            ...
            <Card title="Installed Product Search">
              <div className="mapFilters">
                <label>Search
                  <input placeholder="Serial, Customer, PO, Invoice, City, State, TeamViewer..." value={installedSearch} onChange={e=>setInstalledSearch(e.target.value)} />
                </label>
                <label>Year
              ...
              <div className="summaryCards">
                <KPI title="Search Results" value={installedRowsForList().length} icon="🔎"/>
                <KPI title="Installed Years" value={[...new Set(installedRowsForList().map(installYear))].length} icon="📅"/>
                <KPI title="States" value={[...new Set(installedRowsForList().map(p=>p.state).filter(Boolean))].length} icon="🗺️"/>
              </div>
              <Table headers={['Year', ...PRODUCT_MODELS]} rows={yearlyProductSummary(installedRowsForList()).map(([year,items])=>[year, ...PRODUCT_MODELS.map(p=>items[p] || 0)])}/>
            </Card>

            <Card title="Add Support History">
              <form onSubmit={createSupportHistory} className="opsForm labeledOpsForm">
                <label>Search Installed Product
                  <input placeholder="Search by serial, customer, PO, invoice, TeamViewer..." value={supportProductSearch} onChange={e=>setSupportProductSearch(e.target.value)} />
                </label>
                <label>Selected Product
                  <select value={supportForm.installed_product_id} onChange={e=>setSupportForm({...supportForm,installed_product_id:e.target.value})}>
                    <option value="">Select from search results</option>
                    {supportProductSearchResults().map(p=><option key={p.id} value={p.id}>{p.serial_number} · {p.customer_name} · {p.product_model}</option>)}
                  </select>
                </label>
                <label>Support Date
                ...
                <label>Support Type
                  <select value={supportForm.support_type} onChange={e=>setSupportForm({...supportForm,support_type:e.target.value})}>
                    <option>Remote</option><option>On-site</option><option>Phone</option><option>Email</option><option>Training</option>
                  </select>
                </label>
                <label>Support Summary
                  <input placeholder="Short summary" value={supportForm.support_summary} onChange={e=>setSupportForm({...supportForm,support_summary:e.target.value})}/>
                </label>
                <label>Support Notes
                  <input placeholder="Detailed notes" value={supportForm.support_notes} onChange={e=>setSupportForm({...supportForm,support_notes:e.target.value})}/>
                </label>
                <button>Add Support Log</button>
              </form>
              <p className="hint">Tip: search by serial number, customer name, PO number, invoice number, or TeamViewer ID.</p>
            </Card>

            <Card title="Installed Products List">
              <Table headers={['Model','Serial','Customer','PO','Invoice','TeamViewer','City','State','Installed','Warranty End','Support','Status','Action']} rows={installedRowsForList().map(p=>[
                p.product_model,p.serial_number,p.customer_name,p.po_number,p.invoice_number,
                p.teamviewer_id ? `${p.teamviewer_id}${p.teamviewer_password ? ' / ' + p.teamviewer_password : ''}` : '-',
                p.city,p.state,p.install_date,p.warranty_end,supportRowsForProduct(p.id).length,p.status,
                <div className="actions" key={p.id}><button className="light" onClick={()=>setSelectedInstalledProductId(p.id)}>View Support</button><button className="light" onClick={()=>editInstalledProduct(p)}>Edit</button>{isAdmin && <button className="miniDelete" onClick={()=>deleteInstalledProduct(p.id)}>×</button>}</div>
              ])}/>
            </Card>

            <Card title="Selected Equipment Support History">
              {selectedInstalledProduct() ? (
                <>
                  <p className="hint">Showing support history for {selectedInstalledProduct().serial_number} · {selectedInstalledProduct().customer_name} · {selectedInstalledProduct().product_model}</p>
                  <Table headers={['Date','Type','Summary','Notes','Handled By']} rows={selectedSupportRows().map(s=>[s.support_date,s.support_type,s.support_summary,s.support_notes,s.handled_by_name])}/>
                </>
              ) : <Empty text="Click View Support from Installed Products List to see equipment-specific support history."/>}
            </Card>
          </>
        }
