...
alter table installed_products add column if not exists po_number text;
alter table installed_products add column if not exists invoice_number text;
alter table installed_products add column if not exists teamviewer_id text;
alter table installed_products add column if not exists teamviewer_password text;
alter table installed_products add column if not exists teamviewer_notes text;
alter table installed_products add column if not exists teamviewer_updated_at timestamptz;

create table if not exists support_history (
  id uuid primary key default gen_random_uuid(),
  installed_product_id uuid references installed_products(id) on delete cascade,
  serial_number text,
  customer_name text,
  product_model text,
...
drop policy if exists "support_history_all_authenticated" on support_history;

create policy "support_history_all_authenticated"
on support_history for all
using (true)
with check (true);
